...
/**
 * Figma "Week" — one muted card of seven rows. A day with no window is closed: its
 * name is struck through and both halves drop to the muted tone, which is how the
 * frame distinguishes "nothing configured" from "configured as unavailable".
 */
function WeekTable({
  windows,
}: {
  readonly windows: AvailabilityProfileFixture["windows"];
}) {
  const t = useTranslations("availability");

  return (
    <div className="flex w-full shrink-0 flex-col items-start overflow-clip rounded-lg bg-muted">
